...
class radius_fusion {
 private:
  ...
  double maxsigma_kpx, minsigma_kpx, maxsigma_kpy, minsigma_kpy, maxsigma_deg,
      minsigma_deg;
  int switch_radius, inner_switch;
 ...
 public:
  explicit radius_fusion(
      const double _radius_pos[2], const double _target_pos[2],
      const int &_sample_num, const int &_switch_radius,
    ...
    std::cout << "switch_radius " << switch_radius << std::endl;
  }

  void callback(const sensor::radius_msgs &msg) {
    ...
    //       temp_y + radius_pos[1];
    // kpx = target_pos[0] + target_pos[1] + temp_x + radius_pos[0];
    // kpy = target_pos[1] - target_pos[0] + temp_y + radius_pos[1];
    kpx = -radius_pos[0] * cos(temp_deg / 180 * M_PI) -
          radius_pos[1] * sin(temp_deg / 180 * M_PI) + target_pos[0] + temp_x;
    kpy = -radius_pos[1] * cos(temp_deg / 180 * M_PI) +
          radius_pos[0] * sin(temp_deg / 180 * M_PI) + target_pos[1] + temp_y;
    ...
    std::cout << "radius_pos[0]     " << radius_pos[0] << std::endl;
    std::cout << "radius_pos[1]     " << radius_pos[1] << std::endl;

    kpdeg = temp_deg;
    for (int i = 0; i < sample_num - 1;
         i++) {  // update the data of radius1 kpy
  ...
  void update_parameter(
      const double new_radius_pos[2], const double new_target_pos[2],
      const int &new_switch_radius,  //这部分是在实现什么功能呢
    ...
    radius_pos[0] = new_radius_pos[0];
    radius_pos[1] = new_radius_pos[1];
    target_pos[0] = new_target_pos[0];
    target_pos[1] = new_target_pos[1];
    switch_radius = new_switch_radius;
