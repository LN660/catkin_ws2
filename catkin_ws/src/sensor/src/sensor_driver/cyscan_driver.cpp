...
#include "cmath"
#include "ros/ros.h"
#include "sensor/cyscan_msgs.h"  //定义的消息类型记得修改
using namespace std;
const double Pi = 3.14159265358979;

int main(int argc, char** argv) {
  ros::init(argc, argv, "cyscan_driver");  //解析参数，命名节点
  ros::NodeHandle nh;                      //创建句柄，实例化node
  ros::NodeHandle private_nh("~");
  sensor::cyscan_msgs msg;  //创建消息
  serial::Serial sp;
  string _port;
  int _baud;
  ...
  ros::Publisher pub =
      nh.advertise<sensor::cyscan_msgs>("raw_cyscan_data", 1);  //创建publisher
  ...
  //判断串口是否打开成功
  if (sp.isOpen()) {
    ROS_INFO_STREAM("cyscan sensor is opened.");
  ...
  while (ros::ok()) {
    if (sp.available()) {
      ROS_INFO_STREAM("Reading from serial port for cyscan_sensor");
      string t_serial_buffer("cyscan error");
      t_serial_buffer = sp.readline(50);
      ...
      ROS_INFO(
          "publish cyscan_info:cyscan_x:%lf m  cyscan_y:%lf m  cyscan_deg:%lf",
          msg.x, msg.y, msg.deg);
      ...
      // msg.kp_deg = msg.deg;
      // ROS_INFO("trans cyscan_info:cyscan_x:%lf  cyscan_y:%lf  cyscan_deg:%lf
      // ",
